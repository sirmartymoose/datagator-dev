...
getHot_colHeaders = function(isOwner){
        ...
        for(var i = 0; i < dSchemaLength; i++){
                ...
                if(isOwner == true){
                        colHeaders[0] = 'Email Address <img src="/qa.png" id="help" data-toggle="tooltip" data-placement="right" title="Enter email address of person to share this row with"/>'
                }
        return colHeaders
        }
...
getHot_dataSchema = function(isOwner){
        dataSchema = {sheetId: Session.get('mySheetId')}
        ...
        if(isOwner == false){
                dataSchema['sheetOwnerAuthor'] = Meteor.user()['emails'][0]['address']
        }
        return dataSchema
...
getHot_columns = function(isOwner){
        ...
        for (var i = 0; i < numCols; i++) {
                ...
                if(isOwner === true){
                        ...
                        readOnly = false;
                        cellRender = "text"
                        hiddenMatch = whoView.match(/owner/)
                        ...
                        if(hiddenMatch !== null){
                                cellRender = function (instance, td, row, col, prop, value, cellProperties) {
                Handsontable.renderers.TextRenderer.apply(this, arguments);
                td.style.backgroundColor = "#77bcd9";
